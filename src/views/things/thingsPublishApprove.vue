<template>
  <div class="rightBox">
    ...
    <el-table v-loading="tableLoading" :data="tableInfo">
      ...
      <el-table-column label="发布时间" align="center" prop="time"/>
      ...
      <el-table-column label="审批备注" align="center" v-slot="scope">
        <el-button type="text" v-if="scope.row.auditStatus!==0" @click="showAuditMsg(scope.row.auditMsg)">查看
        </el-button>
      </el-table-column>
    ...
    <template>
      <dialog-things-approve ref="dialogThingsApprove"></dialog-things-approve>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      axios.post('/api' + '/things/getPublishList')
    ...
    approve(data, agree) {
      ...
      this.$refs['dialogThingsApprove'].confirmListener = (msg) => {
        that.loading = true
        axios.post('/api' + '/things/approvePublish', {
          id: data.id,
          auditMsg: msg,
          agree: agree,
