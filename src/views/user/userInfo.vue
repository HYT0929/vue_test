<template>
  <div class="rightBox">
    ...
    <template>
      <!--弹框组件开始-----------------------start-->
    <dialog-new-user v-if="showDialog" ref="dialogNewUser" :dialog-title="dialogTitle" :item-info="userInfoItem" @closeDialog="closeDialog">
    </dialog-new-user>
    <!--弹框组件结束-----------------------end-->
    <!-- 删除提示框-----------------------start-->
...
import DialogNewUser from '../little/dialogNewUser.vue'
      ...
      this.$nextTick(() => {
        ...
        this.$refs['dialogNewUser'].confirmListener = (formInfo) => {
          ...
          axios.post('/api' + '/user/register', {
            ...
            if (res.data.code === 1) {
              this.$refs['dialogNewUser'].showDialog = false
